  ...
  cimg_forXY(img,x,y){           // recorro la imagen
    if (img(x,y)==blanco){       // si es blanco
      ...
      if (vecinos.empty()) {     // si no tengo vecinos etiquetados debo generar nueva etiqueta
        ...
        for (unsigned int i=0; i<vecinos.size()-1; i++)  // controlo la lista de etiquetas vecinas
...
CImg<double> hough(CImg<double> img, bool inverse=false) {
  ...
  if (!inverse){ 
    ...
    cimg_forXY(img,y,x){
      if (img(y,x) > 0.5)
        for (unsigned int t=0; t<M; t++) { //calculo rho variando theta en todo el eje, con x e y fijo
...
///****************************************
/// Etiquetado mediante crecimiento de regiones
///****************************************
CImg<int> label_rg(CImg<int> img, int blanco=1, int delta=1 ,int nueva_etiqueta=1){
